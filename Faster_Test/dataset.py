...
class PotHoleDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        #get the areaa of the bounding boxes
...
def collate_fn(batch):
    """
    This function helps when we have diffrent number of object instanses
    in the batches in the dataset
    """
    return tuple(zip(*batch))
    
# function for the image transforms. 
# Using the outcommented transformations will make the model more robust but slot training time
def train_transform():
     return A.Compose([
        A.Flip(0.5),
...
input = "input\\"
input_dir = os.path.join(faster_dir , input)
csv = "train_df.csv"
csv_dir = os.path.join(input_dir , csv)
train_df = pd.read_csv(csv_dir)
print(train_df.head())
print(f"Total number of image IDs (objects) in the datafram: {len(train_df)}")
